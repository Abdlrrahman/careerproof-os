        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-2 border-b border-slate-200 dark:border-slate-800">
          <div>
            ...
            <h1 className="text-2xl sm:text-3xl font-black text-slate-900 dark:text-white tracking-tight">
              Candidate Executive Dossier & Verified Brief
            </h1>
          </div>
        ...
        <div className="p-8 sm:p-12 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-xl space-y-8 print:border-none print:shadow-none print:p-0">
            ...
            <div className="space-y-2">
              ...
              <div className="flex flex-wrap items-center gap-3 text-xs text-slate-500 font-mono pt-1">
                <span className="flex items-center gap-1">
                  ...
                  <span>Tripoli & Doha (UTC+2 / UTC+3)</span>
                </span>
                <span className="flex items-center gap-1">
                  <Clock className="w-3.5 h-3.5" />
                  <span>8+ hrs EMEA / 5+ hrs US-East Overlap</span>
                </span>
                <span className="flex items-center gap-1 text-emerald-600 font-bold">
                  <ShieldCheck className="w-3.5 h-3.5" />
                  <span>Full Remote Availability</span>
                </span>
              </div>
            </div>

            <div className="text-right space-y-1 text-xs font-mono">
              <div className="text-slate-600 dark:text-slate-300">Target Remote Roles:</div>
              <div className="font-bold text-slate-900 dark:text-white">AI / ML Eng &bull; Staff Backend &bull; Tech Lead</div>
              <div className="text-emerald-600 font-bold pt-1">Target Comp: $150k - $190k USD</div>
            </div>
          </div>
          ...
          <div className="space-y-2">
            ...
            <p className="text-xs sm:text-sm text-slate-700 dark:text-slate-300 leading-relaxed font-medium">
              Founder & Senior Engineering Lead with over six years of production experience architecting high-throughput backend services, spatial ML intelligence platforms (FastAPI, PyTorch, GeoBERT, PostGIS), and multi-tenant enterprise systems (Omega ERP). Proven track record leading multidisciplinary squads of up to 15 engineers, maintaining zero-regression delivery cycles, and advising public-sector and energy consulting clients across EMEA.
            </p>
          </div>
          ...
          <div className="space-y-3">
            <h3 className="text-xs font-bold uppercase tracking-wider text-slate-400">
              Selected Production System Architectures
            </h3>
              ...
              {projects.slice(0, 4).map(p => (
                <div key={p.id} className="p-4 rounded-2xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 space-y-2">
                  <div className="flex justify-between items-start">
                    <strong className="text-slate-900 dark:text-white font-bold">{p.titleEn}</strong>
                  ...
                  <div className="flex flex-wrap gap-1 pt-1">
                    {p.domainTags.slice(0, 4).map((tag, idx) => (
                      <span key={idx} className="text-[10px] font-mono px-2 py-0.5 rounded bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700">
                        {tag}
                      </span>
                    ))}
                  </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-2">
              ...
              <div className="space-y-2 text-xs">
                {achievements.slice(0, 4).map(a => (
                  <div key={a.id} className="p-3 rounded-xl bg-emerald-50/60 dark:bg-emerald-950/30 border border-emerald-200 dark:border-emerald-900/50 flex items-start gap-2">
                    ...
                    <div>
                      <strong className="text-slate-900 dark:text-white">{a.metricValue} {a.metricLabelEn}</strong>
                      <p className="text-slate-600 dark:text-slate-400 mt-0.5">{a.descriptionEn}</p>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="space-y-3">
              <h3 className="text-xs font-bold uppercase tracking-wider text-slate-400">
                Education & Accredited Credentials
              </h3>
              <div className="space-y-2 text-xs">
                {credentials.slice(0, 4).map(c => (
                  <div key={c.id} className="p-3 rounded-xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 flex items-start justify-between gap-2">
                    <div>
                      <strong className="text-slate-900 dark:text-white block">{c.titleEn}</strong>
                      <span className="text-slate-500">{c.issuerEn} &bull; {c.issueDate}</span>
                    </div>
          ...
          <div className="pt-6 border-t border-slate-100 dark:border-slate-800 flex flex-col sm:flex-row sm:items-center justify-between gap-4 text-xs font-mono">
            <div className="text-slate-500">
              Interactive Evidence Graph & Portfolios: <a href="https://careerproof.abdlrrahman.dev" className="text-blue-600 dark:text-cyan-400 underline">https://careerproof.abdlrrahman.dev</a>
            </div>
